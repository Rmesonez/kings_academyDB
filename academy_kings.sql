...
-- para la edad
SELECT users.firstname, users.lastname, users.dob, TIMESTAMPDIFF(YEAR, users.dob, CURDATE()) AS age
FROM users
WHERE users.id = 1;

--otra forma
SELECT users.firstname, users.lastname, users.dob, YEAR(CURDATE()) - YEAR(users.dob) AS age
FROM users
WHERE users.id = 1;

-- no funciono ninguno tampoco
